...
默认把 `wiki/index.md` 视为**活跃知识入口**。若某页面 frontmatter 标记为 `status: retired`，即使它被搜索或反向链接命中，也默认跳过，不作为当前可信依据。

例外：用户明确说“包含退役”“查历史”“为什么退役”“历史上怎么记录”时，可读取 retired 页面，但回答中必须标明这是退役内容，不得与 active 结论混写。

### 步骤 2：深度阅读目标文件

选取步骤 1 中找到的最相关页面，使用读取工具获取完整内容。

读取页面后先检查 frontmatter：

| 状态 | 处理 |
|------|------|
| `status: active` 或缺省 | 可作为当前知识依据 |
| `status: retired` | 默认跳过；仅在用户明确查历史或包含退役内容时引用 |
| `status: needs_review` | 可读，但回答必须提示该页面待复核，不得作为强结论 |

如果一个 active 页面链接到 retired 页面，只能把该链接视为待治理信号；不要沿着它继续展开成当前依据。
...
- **默认跳过退役页面**：`status: retired` 页面不参与当前可信回答；用户明确要求包含退役或查历史时除外
...
- [[lint]] — 结构健康检查
- [[retire]] — 下架过时知识；query 默认跳过 retired 页面
